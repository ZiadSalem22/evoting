...
class ConductBallot extends Component {
  ...
  validateForm = () => {
    ...
    if (!privateKey || privateKey.trim() === "") {
      ...
      privateKey.length <
      "55685527491970eb3000f6cd279e43151cb854fb2fa2c44e23ffb985c841d850".length
    ) {
    ...
    if (
      ...
      newErrors.voteOption = "please select a valid option for poll this poll";
    }
  ...
  updatePolls = (event) => {
    ...
    if (pollId !== "Select Poll") {
      this.setState({
        options: this.state.polls.find((poll) => poll.id === pollId).output
          .options,
      });
    }
  };
        ...
        <Form className="ConductBallotForm">
          ...
          <FormGroup controlId="privateKey">
            ...
            <FormControl
              ...
              onChange={(e) =>
                this.setField("privateKey", e.target.value.trim())
              }
          ...
          <Button onClick={this.fetchValidToVotePolls}>Get polls</Button>

          <br />

          <Form.Group controlId="pollId" hidden={this.state.polls.length < 1}>
            <Form.Label>
              Available Polls [{`${this.state.polls.length}`}]:
            </Form.Label>
            <Form.Control
              className={!!this.props.errors.pollId && "red-border"}
              as="select"
            ...
            >
              ...
              {Object.values(this.state.polls).map((poll) => {
                if (
                  ...
                  //disabled
                  return (
                    <option key={poll.id} value={poll.id} disabled>
                      {`${
          ...
          >
            ...
            <div className="red">{this.props.errors.voteOption}</div>
          </Form.Group>
          <br />
          <div>
            <Button
